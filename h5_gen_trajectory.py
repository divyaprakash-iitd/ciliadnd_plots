...
def create_visualization():
    ...
    N_INSTANCES = 10  # Number of time instances to plot
    
    # Load mesh data
    Lx, Ly = 0.01, 0.002
    
    # Open the HDF5 file
    with h5py.File("database.h5", "r") as f:
        # Get cilia data
        cilia_group = f["cilia"]
    ...
    time_indices = np.linspace(0, nFiles-1, N_INSTANCES, dtype=int)
    ...
    # Plot trajectory
    ax.plot(trajectory_x, trajectory_y, 'g-.', linewidth=1.0, label='Particle Trajectory')
    
    # Add arrowheads
    narrows = 5
    add_arrowheads(ax,trajectory_x,trajectory_y,narrows)
    ...
    for idx, alpha in zip(time_indices, alphas):
        ...
        for j in range(ncilia):
            ...
            ax.plot(cilia_x, cilia_y, 'g-', linewidth=2, alpha=alpha)
